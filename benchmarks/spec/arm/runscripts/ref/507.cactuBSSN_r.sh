#!/bin/bash
echo "This is a script for 507.cactuBSSN_r.ref with $1 copies (parallel execution)"
...
for ((i = 0; i < $1; i++)); do
    ...
    if [ $i -eq 0 ]; then
        (
            ...
            ./cactusBSSN_r_base.mytest-64 spec_ref.par
        ) &
    else
        (
            cd $BASE_DIR/run_base_refrate_mytest-64.$extend_i
            ./cactusBSSN_r_base.mytest-64 spec_ref.par > /dev/null 2>&1
        ) &
    fi
done
