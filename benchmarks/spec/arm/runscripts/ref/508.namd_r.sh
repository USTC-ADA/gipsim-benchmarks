...
for ((i = 0; i < $1; i++)); do
    ...
    if [ $i -eq 0 ]; then
        (
            ...
            ./namd_r_base.mytest-64 --input apoa1.input --output apoa1.ref.output --iterations 65
        ) &
    else
        (
            cd $BASE_DIR/run_base_refrate_mytest-64.$extend_i
            ./namd_r_base.mytest-64 --input apoa1.input --output apoa1.ref.output --iterations 65 > /dev/null 2>&1
        ) &
    fi
done
