...
for ((i = 0; i < $1; i++)); do
    ...
    if [ $i -eq 0 ]; then
        ...
        (
            cd $BASE_DIR/run_base_refrate_mytest-m64.$extend_i
            ./imagick_r_base.mytest-m64 -limit disk 0 refrate_input.tga -edge 41 -resample 181% -emboss 31 -colorspace YUV -mean-shift 19x19+15% -resize 30% refrate_output.tga > /dev/null 2>&1
        ) &
    fi
done
