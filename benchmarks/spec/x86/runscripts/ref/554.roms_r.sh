#!/bin/bash
echo "This is a script for 554.roms_r.ref with $1 copies (parallel execution)"
...
for ((i = 0; i < $1; i++)); do
    ...
    if [ $i -eq 0 ]; then
        (
            ...
            ./roms_r_base.mytest-m64 < ocean_benchmark2.in.x
        ) &
    else
        (
            cd $BASE_DIR/run_base_refrate_mytest-m64.$extend_i
            ./roms_r_base.mytest-m64 < ocean_benchmark2.in.x > /dev/null 2>&1
        ) &
    fi
done
