#!/bin/bash
echo "This is a script for 511.povray_r.test with $1 copies (parallel execution)"
...
for ((i = 0; i < $1; i++)); do
    ...
    if [ $i -eq 0 ]; then
        (
            ...
            ./povray_r_base.mytest-m64 SPEC-benchmark-test.ini
        ) &
    else
        (
            cd $BASE_DIR/run_base_test_mytest-m64.$extend_i
            ./povray_r_base.mytest-m64 SPEC-benchmark-test.ini > /dev/null 2>&1
        ) &
    fi
done
