...
for ((i = 0; i < $1; i++)); do
    ...
    if [ $i -eq 0 ]; then
        (
            ...
            ./blender_r_base.mytest-m64 cube.blend --render-output cube_ --threads 1 -b -F RAWTGA -s 1 -e 1 -a
        ) &
    else
        (
            cd $BASE_DIR/run_base_test_mytest-m64.$extend_i
            ./blender_r_base.mytest-m64 cube.blend --render-output cube_ --threads 1 -b -F RAWTGA -s 1 -e 1 -a > /dev/null 2>&1
        ) &
    fi
done
