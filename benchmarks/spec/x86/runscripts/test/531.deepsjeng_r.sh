#!/bin/bash
echo "This is a script for 531.deepsjeng_r.test with $1 copies (parallel execution)"
...
for ((i = 0; i < $1; i++)); do
    ...
    if [ $i -eq 0 ]; then
        (
            ...
            ./deepsjeng_r_base.mytest-m64 test.txt
        ) &
    else
        (
            cd $BASE_DIR/run_base_test_mytest-m64.$extend_i
            ./deepsjeng_r_base.mytest-m64 test.txt > /dev/null 2>&1
        ) &
    fi
done
