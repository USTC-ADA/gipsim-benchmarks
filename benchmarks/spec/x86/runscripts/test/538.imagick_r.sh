...
for ((i = 0; i < $1; i++)); do
    ...
    if [ $i -eq 0 ]; then
        (
            ...
            ./imagick_r_base.mytest-m64 -limit disk 0 test_input.tga -shear 25 -resize 640x480 -negate -alpha Off test_output.tga
        ) &
    else
        (
            cd $BASE_DIR/run_base_test_mytest-m64.$extend_i
            ./imagick_r_base.mytest-m64 -limit disk 0 test_input.tga -shear 25 -resize 640x480 -negate -alpha Off test_output.tga > /dev/null 2>&1
        ) &
    fi
done
